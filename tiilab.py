...
#
    ...
    try :
        ...
        print(legx)
        print(u'Echec  l\'appel de imz2matbase')
        return 0,0,0,0
    ftest.close()
    
    nparam=np.size(listeparam)
    if(nparam!=7):
        print(u'Echec appel imz2matbase : nombre errone de parametres (%d'%nparam+' au lieu de 7')
        return 0,0,0,0
...
def imz2mat(imgName,ncan=0):
    ...
    argument 2 (facultatif) : si multicanal, renvoie uniquement le canal indique
    """
...
#  
    if imgName.endswith('.dat') : 
        return( dat2mat(imgName ))  # voir a  la fin de ce fichier, avant les outils de visu
...
#    print('parametre ncan : %d'%ncan)
    ...
    if  nkparam<0 :
        print(u'Fichier RNSat : procedure en test')
        nk=-nkparam
        tagRNSAT=1
...
#############                
            if komplex==0:           
                print(u'Donnees reelles. Nouvelle version de imz2mat  '+'%s'%typeA)
...
############  komplex > 0  : trois cas 
            ...
            if komplex==1 or komplex==11:   # cas des complexes standards :
                                            # partie reelle puis partie imaginaire
#                for i in range(0, h):
...
#   imgligneZ =  numpy.ndarray( 2*nfencboucle, '<h', record[4*vigdebcol:4*(vigdebcol+nfencboucle)]) 
                ...
                print(u'Donnees complexes (standard). Nouvelle version de imz2mat  '+'%s'%typeA)
...
## komplex==2                  
            if komplex==2:  # d'abord la partie reelle, puis la partir imaginaire
                for i in range(0, h):
                    for j in range(0, w):
                        imgligne[j] = struct.unpack( typeA, f.read(nbBytes))[0]
                    for j in range(0, w):
...
def dat2mat(imgName):
#
# on rajoute un test dur l'existence meme du fichier
#
    ...
    try :
        ...
        legx=imgName+': est un fichier non ouvrable'
        print(legx)
        print(u'Echec de  l\'appel de dat2mat')
        return 0,0,0,0
...
# on elimine la premiere ligne : cf la doc !!
...
def visusar(tabima,zparam=-999):
    ...
    plt.show() incorpore dans cette routine
    
    Arguments en entr̩e : 1 ou 2
    
        argument 1 : tableau 2D image
        
        argument 2 (facultatif) : facteur de la formule <<valeur moyenne + fak * ecart type >>
        Si ce facteur est nul, l'image ne sera pas seuillee
        
    Argument en sortie : le tableau affiche (avec seuillage)
        
    Utilisez visusarZ (meme syntaxe) pour eviter le plt.show()
    
    Utilisez visusarW (meme syntaxe) pour n'avoir aucun affichage
    """
...
def mat2imz( tabimage, nomimage, *therest):
    ...
    Le tableau sera archive en :
        .ima si tableau 8 bits
        .IMF sinon
        .CXF si complexe   
    Si le tableau est a  3 dimensions (pile TIVOLI), l'archivage se fera en .IMA
    Exemple d'appel :
